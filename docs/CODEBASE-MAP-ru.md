...
- **`TSpriteCache`** — словарь `имя файла → PSdlTexture`, ленивая загрузка
  PNG. Два источника: базовая папка или `TSpriteSet`, подключённый через
  `AttachSpriteSet` (не владеет — освобождает тот, кто открыл). В режиме
  набора `Get('1.png')` и `Get('1')` — один слот: наборы именуют спрайты,
  а не файлы, поэтому старые вызовы работают без правок. Опциональный
  color key (`SetColorKey` / `DisableColorKey`). Кэш на корень ассетов
  или на набор. `SpriteSetsDir` ('sprites\') живёт здесь — дом ассетной
  подсистемы.
- **`TAnimSet`** (запись) — массивы текстур `Alive[0..7]` и `Death[0..7]`;
  `IsLoaded`. Собирается перегруженной **`LoadAnimSet`**: `(cache, mnsFile)`
  разбирает список спрайтов `.mns`; `(cache, spriteSet)` читает
  последовательности `alive`/`death` манифеста, каждая проверяется на
  ровно восемь кадров.
...
юнит, три вызывающих. Движок читает наборы для монстров, героя, оружия и
пуль; окружение (тайлы, фоны, меню, шрифт) остаётся на россыпи, пока
формат уровня не выучит `spriteSets`.
...
- **`pack-sets.ps1`** — собирает все 29 наборов за один прогон и держит
  ...
  неразобранное пакуется отдельно и докладывается. Тайлы проходят через
  временную папку, потому что игра пока читает россыпь там, где она лежит.
