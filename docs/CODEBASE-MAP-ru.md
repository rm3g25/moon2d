...
- **`TSpriteCache`** — словарь `набор:имя → PSdlTexture`, ленивая загрузка
  из наборов, подключённых через `AttachSpriteSet` (не владеет —
  освобождает тот, кто открыл). Разрешение: квалифицированное имя
  (`common:pustota`) идёт только в названный набор; голое — в первый
  подключённый, который его содержит; имя, которого нет нигде, вызывает
  исключение. Путь и расширение отбрасываются при поиске, поэтому написания
  2008 в палитрах (`level1\doom1.png`) по-прежнему разрешаются.
  `AmbiguousNames` докладывает голые имена, встречающиеся в нескольких
  наборах — они разрешаются порядком объявления, ради чего квалификатор и
  заведён. Опциональный color key (`SetColorKey` / `DisableColorKey`).
  `SpriteSetsDir` ('sprites\') живёт здесь — дом ассетной подсистемы, как
  и свободная функция **`LoadImageSurface`**, единственное место, которое
  превращает хранимые байты в поверхность.
- **`TAnimSet`** (запись) — массивы текстур `Alive[0..7]` и `Death[0..7]`;
  `IsLoaded`. Собирается **`LoadAnimSet(cache, spriteSet)`** из
  последовательностей `alive` и `death` манифеста, каждая проверяется на
  ровно восемь кадров — `TAnimSet` это контракт 2008 с фиксированными
  массивами.
...
### `Sprites.Sets.pas` (~430 строк)
Контейнер наборов спрайтов `.mset`: JSON-манифест, за ним все картинки
подряд. Читается игрой, упаковщиком и (позже) редактором уровней — один
юнит, три вызывающих. Вся графика игры приходит из наборов, россыпь
картинок больше не поставляется. `SetQualifier` (':') живёт здесь же —
редактор и упаковщик читают тот же синтаксис.
- **Константы**: `MsetVersion=1`. Имена полей манифеста — константы
  (`KeyId`, `KeySprites`, `KeyOffset`…): опечатка в литерале компилируется.
- **Записи**: `TSpriteEntry` (имя, описание, offset, size — смещение от
  начала блока картинок, не от начала файла), `TSpriteSequence` (имя,
  описание, кадры), `TMsetHeader` (packed: магия, версия, размер
  манифеста), `TMsetMagic` (именованный тип — анонимный
  `array[0..3] of AnsiChar` не присвоится другому такому же).
- **`TSpriteSet`** — чтение. При открытии разбирается только манифест,
  байты картинок приходят по требованию: `ReadSprite(name)`, `Contains`,
  `SequenceFrames`.
- **`TSpriteSetWriter`** — запись. `AddSprite`/`AddSpriteFile`,
  `AddSequence`, `SaveToFile`. Смещения раздаются при сохранении в порядке
  добавления; запись детерминированная, поэтому неизменившийся набор
  пересобирается байт в байт. Ловит дубли имён и последовательности,
  ссылающиеся на отсутствующие кадры.
- Спека формата: `docs/MSET-FORMAT.md`.

### `Sdl2.Image.pas` (~75 строк)
Биндинги SDL2_image, delayed-импорты по образцу `Audio.pas`.
`IMG_Load_RW` заменил `SDL_LoadBMP_RW` во всех четырёх точках загрузки.
`EnsureImageLib` вызывается на старте и падает внятным сообщением, если
DLL нет: в отличие от опционального миксера, отсутствие графики фатально.
...
Собирает и осматривает `.mset`. Обёртка над `Sprites.Sets`, больше в нём
ничего нет.
- **`SpritePackCli.dpr`** — команды `pack` / `list` / `unpack`. `pack`
  берёт все PNG папки в естественном порядке (2 раньше 10); `--list`
  разбирает список 2008 на именованные последовательности по его длине
  (16 строк → alive+death, 24 → walk+death+henshin, иначе одна группа).
  `unpack` пишет картинки и `manifest.json`, так что набор всегда можно
  разобрать.
- **`pack-sets.ps1`** — собирает все 29 наборов за один прогон и держит
  тематические таблицы тайлов: первый совпавший шаблон забирает имя,
  неразобранное пакуется отдельно и докладывается. Пересобирает наборы из
  рабочей копии россыпи — в поставке её нет, так что это инструмент
  сопровождения, а не шаг сборки.
- VCL-половина (`SpritePack.exe`, правка имён и описаний) запланирована;
  логика остаётся в `Sprites.Sets`, обе обёртки тонкие.
...
| Монстр/герой берёт не те кадры из набора | Monsters.pas AnimFor / Hero.pas LoadSetSequences |
| Упаковка и осмотр наборов | tools/SpritePack/* |
